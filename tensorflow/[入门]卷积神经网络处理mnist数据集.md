...
def weight_variable(shape):
    # tf.truncated_normal(shape, mean, stddev) 产生截断的正态分布
...
def conv2d(x, W):
    # tf.nn.conv2d(input, filter, strides, padding, use_cudnn_on_gpu=None, name=None) 卷积
    return tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME')

def max_pool_2x2(x):
    # tf.nn.max_pool(value, ksize, strides, padding, name=None) 池化
